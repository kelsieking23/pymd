...
class Ligand:
    ...
    def __init__(self, structure, name=None, ignh=False):
        self.structure = structure
        self.name = name
        self.ignh = ignh
        # self.atom_coordinates = self.getAtomCoordinates()
        self.ligands = self.getLigand()
        self.lig = self.ligands[0]
    ...
    def getLigand(self):
        ...
        for line in f:
            if self.name in line:
                ...
                res_id = line_parts[3] + line_parts[5]
                if last_res_id is None:
                    data.append(line)
                elif last_res_id != res_id:
                    lig = SingleLigand(data)
                    ligands.append(lig)
                    data = []
                    data.append(line)
                else:
    ...
    def __init__(self, structure, name=None, covalent_id=None, model=None, ignh=False, alt=None):
        ...
        ** alt (optional):  string. Gives ligand an alternative name, which can be used in certain functions. 
        ...
        if name is not None:
            ...
            try:
                self.name = structure.split('/')[-1].split('.')[0]
            except:
                self.name = structure.split('.')[0]
