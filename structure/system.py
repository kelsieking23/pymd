...
class System:
    ...
    ** model (optional): string. Model coordinates to use as ligand coordinates if multiple models are in the ligand structure file. Default is MODEL 1.
    ...
    ** ignh (optional): boolean. Specifies if hydrogen should be ignored when collecting coordinates. Default is false
    '''
    def __init__(self, protein, ligands=None, residues=None, covalent_ids=None, model=None, names=None, alt=None, ignh=False):
                ...
                if names is not None:
                    if isinstance(names, list):
                        for i in range(len(ligands)):
                            if alt is not None:
                                self.ligands.append(Ligand(structure=ligands[i], alt=alt[i], model=model, name=names[i], ignh=ignh))
                            else:
                                self.ligands.append(Ligand(structure=ligands[i], model=model, name=names[i], ignh=ignh))
                    if isinstance(names, dict):
                        for key in names.keys():
                            name = names[key]
                            self.ligands.append(Ligand(structure=key, model=model, name=name, ignh=ignh))
                        loaded = []
                        for ligand in self.ligands:
                            loaded.append(ligand.structure)
                        for ligand in ligands:
                            if ligand not in loaded:
                                self.ligands.append(Ligand(structure=ligand, model=model, ignh=ignh))
                else:
                    for ligand in ligands:
                        self.ligands.append(Ligand(structure=ligand, model=model, ignh=ignh))
            else:
