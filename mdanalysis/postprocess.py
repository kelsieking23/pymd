...
class PostProcess:
            ...
            for f in files:
                ...
                if (regex.findall(filename) != []) and (filename.count('.') < 2):
                    ...
                    if self.system.ligands is not None:
                        nterm = self.system.protein.nterm
                        cterm = self.system.protein.cterm
                        editChainIDResidue(new, new, nterm, cterm, sm=self.system.ligands)
      
    def clusterPercentages(self, period=(400,600)):
        df = pd.DataFrame()
        for rep in range(1, self.system.reps+1):
    ...
    def dsspPerResidue(self, gro, xpm, rep, peptides=None):
        # PATH = 'C:\\Users\\KelsieKing\\Desktop\\Work\\iapp\\pymd\\mdanalysis\\'
        PATH = os.getcwd()
    ...
    def dsspOverTime(self, xvg, rep=None, block_average=2, average=None, tu='ns', average_replicates=False):
        '''
        Post-processing for dssp over time data. Writes CSV.
        Arguments:
        ** xvg (str): the .xvg base name (example: 'dssp_0_600')
        ** block_average (int, optional, default=2): frames to block average by
        ** average (tup, optional): timeframe to take average over (example: (400,600))
        ** tu (str, optional, default='ns'): if average is given, must specify what time units. default is nanosecond ('ns')
        Returns:
        ** pandas DataFrame
        '''
        ...
        if convert is True:
            df.index = [i/1000 for i in df.index]
        df['coil_percent'] = ((df['Coil'] + df['Bend'] + df['Turn']) / 30) * 100 
        df['bsheet_percent'] = ((df['B-Sheet'] + df['B-Bridge']) / 30) * 100
        df['helix_percent'] = ((df['3-Helix'] + df['A-Helix'] + df['5-Helix']) / 30) * 100
        df['total'] = df['coil_percent'] + df['bsheet_percent'] + df['helix_percent']
        ...
        #     avg = df.loc[average[0]:average[1],:]
        #     coil_avg = df['coil_percent'].mean()
        #     beta_avg = df['bsheet_percent'].mean()
        #     helix_avg = df['helix_percent'].mean()
        #     avg = (coil_avg, beta_avg, helix_avg)
        # else:
        #     avg = None
