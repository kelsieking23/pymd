...
class Plotter:
    ...
    def timeseries(self, pdata):
        ...
        for d in pdata.data:
            ax.plot(d.x, d.y, color=d.color, label=d.label)
        ...
        if pdata.legend is not None:
            if hasattr(pdata.legend, 'fontsize'):
        ...
        if pdata.annotations is not None:
            for annotation in pdata.annotations:
                ...
                if annotation.atype == 'annotate':
                    plt.annotate(text=annotation.text, xy=annotation.xy, fontsize=annotation.fontsize)
    ...
    def timeseriesPanel(self, data, output, title, legend_data=None):
        ...
        if h > 1:
            for x in range(0, h):
                for y in range(0, w):
                    ...
                    for d in pdata.data:
                        ax[x,y].plot(d.x, d.y, color=d.color, label=d.label)
                        if hasattr(d, 'fill_between'):
                            ax[x,y].fill_between(d.x, d.y-d.fill_between, d.y+d.fill_between, alpha=0.2, color=d.color)
            ...
            for x in range(0, w):
                ...
                for d in pdata.data:
                    print(d.y)
                    ax[x].plot(d.x, d.y, color=d.color, label=d.label)
                    if hasattr(d, 'fill_between'):
                        ax[x].fill_between(d.x, d.y-d.fill_between, d.y+d.fill_between, alpha=0.2, color=d.color)
                ...
                # legend
                if pdata.legend is not None:
                    ax[x].legend(loc=pdata.legend.loc)
        ...
        if legend_data is not None:
            ...
            for label, color in legend_data:
                ...
                labels.append(label)
            plt.figlegend(handles, labels, loc='lower center', ncol=3, fontsize=14)
    ...
    def markerPlot(self, pdata):
        ...
        for d in pdata.data:
            errorbars = ax.errorbar(x=d.x, y=d.y, xerr=None, yerr=d.yerr, marker='o',capsize=5, elinewidth=1, capthick=1, ms=7, fillstyle='full', fmt='none', color=d.color, label=None)
            points = ax.scatter(d.x, d.y, s=d.s, color=d.color, label=d.label)
        ...
        if pdata.legend is not None:
            if (hasattr(pdata.legend, 'individual')):
                if pdata.legend.individual is True:
                    ax.legend(loc=pdata.legend.loc, markerscale=pdata.legend.markerscale, fontsize=14)
                else:
                    pass
            else:
                ax.legend(loc=pdata.legend.loc, markerscale=pdata.legend.markerscale, fontsize=14)
    ...
    def markerPlotPanel(self, data, output, title, legend_data=None):
        ...
        for x in range(0, h):
            for y in range(0, w):
                ...
                try:
                    ...
                    continue
                for d in pdata.data:
                    errorbars = ax[x,y].errorbar(x=d.x, y=d.y, xerr=None, yerr=d.yerr, marker='o',capsize=5, elinewidth=1, capthick=1, ms=7, fillstyle='full', fmt='none', color=d.color, label=None)
                    points = ax[x,y].scatter(d.x, d.y, s=d.s, color=d.color, label=d.label)
        ...
        if legend_data is not None:
            ...
            for label, color, size in legend_data:
                ...
                labels.append(label)
            plt.figlegend(handles, labels, loc='lower center', ncol=3, fontsize=18)
    ...
    def barPlot(self, pdata):
        ...
        if not isinstance(pdata.data, list):
            if pdata.data.color is None:
                ...
                rect = ax.bar(pdata.data.xpos, pdata.data['mean'], yerr=pdata.data['stdev'], width=pdata.data.width, label=pdata.data.label, ecolor='#5c5c5c', capsize=pdata.data.capsize, color=pdata.data.color)
            rects.append(rect)
        else:
            ...
            for annotation in pdata.annotations:
                if annotation.atype == 'autolabel':
                    for rect in rects:
                        ...
                        self.autolabel(rect, ax, caplines, fontsize=annotation.fontsize)
        ...
        ax.set_xticks(pdata.xticks.locs)
